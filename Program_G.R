...
vero_pp <- function(alpha){
  ...
  L<-function(x){
    ...
    R=exp(-((exp(alpha2)/alpha1)*(exp(alpha1*x[1])-1)-
               ((exp(alpha2)/alpha1)*(exp(alpha1*x[2])-1))))
    return(R)
  }
  
  l<-function(y){
    #R1=(alpha1/alpha2)*((y/alpha2)^(alpha1-1))
    R1=exp(alpha2+alpha1*y)
    return(R1)
  }
...
#Change as needed.  
P <- optim(c(0.005,-4),  # Initial values
...
P=nlminb(c(0.005,-4),vero_pp,lower=c(0.001,-11),upper=c(0.009,-3))
